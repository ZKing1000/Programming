...
class textBoxInput():
	def __init__(self,screen,clock,pos,endPos,fontType,maxStrSize,backgroundColor,textColor,boarderOffColor,boarderOnColor):
		...
		self.textEnter = "" #handler, rapidEnterFunc, drawBox, textBoxInput
 ...
 #drawBox
	...
	def drawBox(self):
		...
		self.screen.blit(font.render(self.currentText,True,(0,0,0)),self.fontPastePos())

	def textBoxInput(self):
		self.rapidEnterFunc()
		self.drawBox()
		if self.isDone == True: return self.textEnter
...
class textBoxDisplay():
	...
	def textBoxDisplay(self):
		if self.backgroundColor != None:
			pygame.draw.rect(self.screen, self.backgroundColor, Rect(self.pos,((self.endPos[0] - self.pos[0]),(self.endPos[1] - self.pos[1]))))
		...
		for y in range(self.fontStuffs[2]):
			SURFACEFONT = self.font.render(self.fontStuffs[1][y],True,self.textColor)
			self.screen.blit(SURFACEFONT,(self.pos[0] +2,self.pos[1] + (y * self.font.size(self.biggestLetter[1])[1])))
